...
/// @dev Collection of some commonly used utility function for array of type int224
library ArrayInt224 {
     ...
     */
    function sort(int224[] storage _array) internal {
     ...
     */
    function quickSort(
        int224[] storage _array,
        uint256 _low,
        uint256 _high
    ) private {
        if (_low < _high) {
            int224 pivotVal = _array[(_low & _high) + (_low ^ _high) / 2];
            uint256 lv = _low;
            uint256 uv = _high;
            while (true) {
     ...
     * - input array must contain more than 1 (one) element
     */
    function isSorted(int224[] storage _array) internal view returns (bool) {
     ...
     * - input array must contain more than 1 (one) element
     */
    function isSortedDesc(int224[] storage _array)
        internal
        view
        returns (bool)
    {
    ...
    function includes(int224[] storage _array, int224 _value)
        ...
        returns (bool)
    {
        require(_array.length > 0, "ArrayInt224: array should not be empty");
     ...
     */
    function includesInSorted(int224[] storage _array, int224 _value)
        internal
        view
        returns (bool)
    {
     ...
     * - input array must not be empty
     */
    function indexOf(int224[] storage _array, int224 _value)
        internal
        view
        returns (int256)
    {
     ...
     * - input array must be sorted in ascending order.
     */
    function indexOfInSorted(int224[] storage _array, int224 _value)
        internal
        view
        returns (int256)
    {
     ...
     * - input array must not be empty
     */
    function lastIndexOf(int224[] storage _array, int224 _value)
        internal
        view
        returns (int256 r)
    {
     ...
     */
    function at(int224[] storage _array, int256 _index)
        internal
        view
        returns (int224)
    {
        ...
        require(
            index < _array.length,
            "ArrayInt224: index should not be greater than array length"
        );
        return _array[index];
    }
     ...
     * - index must not be greater than the array length
     */
    function remove(int224[] storage _array, uint256 _index) internal {
        require(_array.length > 0, "ArrayInt224: array should not be empty");
        require(

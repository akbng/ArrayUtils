...
/// @dev Collection of some commonly used utility function for array of type int8
library ArrayInt8 {
     ...
     */
    function sort(int8[] storage _array) internal {
    ...
    function quickSort(
        ...
        if (_low < _high) {
            int8 pivotVal = _array[(_low & _high) + (_low ^ _high) / 2];
            uint256 lv = _low;
            uint256 uv = _high;
            while (true) {
     ...
     * - input array must contain more than 1 (one) element
     */
    function isSorted(int8[] storage _array) internal view returns (bool) {
     ...
     * - input array must contain more than 1 (one) element
     */
    function isSortedDesc(int8[] storage _array) internal view returns (bool) {
        require(_array.length > 1, "ArrayUint256: array should not be empty");
        for (uint256 i = 0; i < _array.length - 1; i++) {
            if (_array[i] <= _array[i + 1]) return false;
        }
    ...
    function includes(int8[] storage _array, int8 _value)
        ...
        returns (bool)
    {
        require(_array.length > 0, "ArrayInt8: array should not be empty");
     ...
     */
    function includesInSorted(int8[] storage _array, int8 _value)
        internal
        view
        returns (bool)
    {
     ...
     * - input array must be sorted in ascending order.
     */
    function indexOfInSorted(int8[] storage _array, int8 _value)
        internal
        view
        returns (int256)
    {
     ...
     */
    function at(int8[] storage _array, int256 _index)
        internal
        view
        returns (int8)
    {
        ...
        require(
            index < _array.length,
            "ArrayInt8: index should not be greater than array length"
        );
        return _array[index];
    }
     ...
     * - index must not be greater than the array length
     */
    function remove(int8[] storage _array, uint256 _index) internal {
        require(_array.length > 0, "ArrayInt8: array should not be empty");
        require(

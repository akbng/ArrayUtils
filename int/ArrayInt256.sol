...
/// @dev Collection of some commonly used utility function for array of type int256
library ArrayInt256 {
     ...
     */
    function sort(int256[] storage _array) internal {
     ...
     */
    function quickSort(
        int256[] storage _array,
        uint256 _low,
        uint256 _high
    ) private {
        if (_low < _high) {
            int256 pivotVal = _array[(_low & _high) + (_low ^ _high) / 2];
            uint256 lv = _low;
            uint256 uv = _high;
            while (true) {
     ...
     * - input array must contain more than 1 (one) element
     */
    function isSorted(int256[] storage _array) internal view returns (bool) {
     ...
     * - input array must contain more than 1 (one) element
     */
    function isSortedDesc(int256[] storage _array)
        internal
        view
        returns (bool)
    {
    ...
    function includes(int256[] storage _array, int256 _value)
        ...
        returns (bool)
    {
        require(_array.length > 0, "ArrayInt256: array should not be empty");
     ...
     */
    function includesInSorted(int256[] storage _array, int256 _value)
        internal
        view
        returns (bool)
    {
     ...
     * - input array must be sorted in ascending order.
     */
    function indexOfInSorted(int256[] storage _array, int256 _value)
        internal
        view
        returns (int256)
    {
     ...
     */
    function at(int256[] storage _array, int256 _index)
        internal
        view
        returns (int256)
    {
        ...
        require(
            index < _array.length,
            "ArrayInt256: index should not be greater than array length"
        );
        return _array[index];
    }
     ...
     * - index must not be greater than the array length
     */
    function remove(int256[] storage _array, uint256 _index) internal {
        require(_array.length > 0, "ArrayInt256: array should not be empty");
        require(

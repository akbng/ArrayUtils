...
/// @dev Collection of some commonly used utility function for array of type int16
library ArrayInt16 {
     ...
     */
    function sort(int16[] storage _array) internal {
     ...
     */
    function quickSort(
        int16[] storage _array,
        uint256 _low,
        uint256 _high
    ) private {
        if (_low < _high) {
            int16 pivotVal = _array[(_low & _high) + (_low ^ _high) / 2];
            uint256 lv = _low;
            uint256 uv = _high;
            while (true) {
     ...
     * - input array must contain more than 1 (one) element
     */
    function isSorted(int16[] storage _array) internal view returns (bool) {
     ...
     * - input array must contain more than 1 (one) element
     */
    function isSortedDesc(int16[] storage _array) internal view returns (bool) {
        require(_array.length > 1, "ArrayUint256: array should not be empty");
        for (uint256 i = 0; i < _array.length - 1; i++) {
            if (_array[i] <= _array[i + 1]) return false;
        }
     ...
     */
    function includesInSorted(int16[] storage _array, int16 _value)
        internal
        view
        returns (bool)
    {
     ...
     * - input array must not be empty
     */
    function indexOf(int16[] storage _array, int16 _value)
        internal
        view
        returns (int256)
    {
     ...
     * - input array must be sorted in ascending order.
     */
    function indexOfInSorted(int16[] storage _array, int16 _value)
        internal
        view
        returns (int256)
    {
     ...
     * - input array must not be empty
     */
    function lastIndexOf(int16[] storage _array, int16 _value)
        internal
        view
        returns (int256 r)
    {
     ...
     */
    function at(int16[] storage _array, int256 _index)
        internal
        view
        returns (int16)
    {

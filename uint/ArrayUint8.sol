...
/// @dev Collection of some commonly used utility function for array of type uint8
library ArrayUint8 {
     ...
     */
    function sort(uint8[] storage _array) internal {
     ...
     */
    function quickSort(
        uint8[] storage _array,
        uint256 _low,
        uint256 _high
    ) private {
        if (_low < _high) {
            uint8 pivotVal = _array[(_low & _high) + (_low ^ _high) / 2];
            uint256 lv = _low;
            uint256 uv = _high;
            while (true) {
     ...
     * - input array must contain more than 1 (one) element
     */
    function isSorted(uint8[] storage _array) internal view returns (bool) {
     ...
     * - input array must contain more than 1 (one) element
     */
    function isSortedDesc(uint8[] storage _array) internal view returns (bool) {
        require(_array.length > 1, "ArrayUint256: array should not be empty");
        for (uint256 i = 0; i < _array.length - 1; i++) {
            if (_array[i] <= _array[i + 1]) return false;
        }
    ...
    function includes(uint8[] storage _array, uint8 _value)
        ...
        returns (bool)
    {
        require(_array.length > 0, "ArrayUint8: array should not be empty");
     ...
     */
    function includesInSorted(uint8[] storage _array, uint8 _value)
        internal
        view
        returns (bool)
    {
     ...
     * - input array must be sorted in ascending order.
     */
    function indexOfInSorted(uint8[] storage _array, uint8 _value)
        internal
        view
        returns (int256)
    {
     ...
     */
    function at(uint8[] storage _array, int256 _index)
        internal
        view
        returns (uint8)
    {
        ...
        require(
            index < _array.length,
            "ArrayUint8: index should not be greater than array length"
        );
        return _array[index];
    }
     ...
     * - index must not be greater than the array length
     */
    function remove(uint8[] storage _array, uint256 _index) internal {
        require(_array.length > 0, "ArrayUint8: array should not be empty");
        require(

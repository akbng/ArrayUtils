...
/// @dev Collection of some commonly used utility function for array of type uint16
library ArrayUint16 {
     ...
     */
    function sort(uint16[] storage _array) internal {
     ...
     */
    function quickSort(
        uint16[] storage _array,
        uint256 _low,
        uint256 _high
    ) private {
        if (_low < _high) {
            uint16 pivotVal = _array[(_low & _high) + (_low ^ _high) / 2];
            uint256 lv = _low;
            uint256 uv = _high;
            while (true) {
     ...
     * - input array must contain more than 1 (one) element
     */
    function isSorted(uint16[] storage _array) internal view returns (bool) {
     ...
     * - input array must contain more than 1 (one) element
     */
    function isSortedDesc(uint16[] storage _array)
        internal
        view
        returns (bool)
    {
     ...
     */
    function includesInSorted(uint16[] storage _array, uint16 _value)
        internal
        view
        returns (bool)
    {
     ...
     * - input array must be sorted in ascending order.
     */
    function indexOfInSorted(uint16[] storage _array, uint16 _value)
        internal
        view
        returns (int256)
    {
     ...
     * - index must not be greater than the array length
     */
    function remove(uint16[] storage _array, uint256 _index) internal {
        require(_array.length > 0, "ArrayUint16: array should not be empty");
        require(

...
/// @dev Collection of some commonly used utility function for array of type uint64
library ArrayUint64 {
     ...
     */
    function sort(uint64[] storage _array) internal {
    ...
    function quickSort(
        ...
        if (_low < _high) {
            uint64 pivotVal = _array[(_low & _high) + (_low ^ _high) / 2];
            uint256 lv = _low;
            uint256 uv = _high;
            while (true) {
     ...
     * - input array must contain more than 1 (one) element
     */
    function isSorted(uint64[] storage _array) internal view returns (bool) {
     ...
     * - input array must contain more than 1 (one) element
     */
    function isSortedDesc(uint64[] storage _array)
        internal
        view
        returns (bool)
    {
     ...
     */
    function includesInSorted(uint64[] storage _array, uint64 _value)
        internal
        view
        returns (bool)
    {
     ...
     * - input array must be sorted in ascending order.
     */
    function indexOfInSorted(uint64[] storage _array, uint64 _value)
        internal
        view
        returns (int256)
    {
